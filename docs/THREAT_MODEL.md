...
| Local AI is loopback-only | `app/core/providers.py`, `app/core/ai/ollama_provider.py` | Ollama is contacted at `127.0.0.1:11434` only; there is deliberately no setting for a remote host. A model the running instance does not report is refused by name rather than silently substituted. |
| A model download starts only from a user action, from one module | `app/core/local_ai_models.py`, `app/api/local_ai_routes.py` | This row used to read "and never downloads". The product's owner reversed that decision; the containment replacing it is enforced rather than described. `/api/pull` may appear only in `local_ai_models.py`, and `model_puller.start()` may be called only from the session-token-protected `POST /local-ai/pull` — both checked by AST tests over every module under `app/`. `GET /local-ai/plan` answers "what would this download?" without downloading it. Nothing runs on startup, on a status read, or as a side effect. |
| A downloaded executable is verified before it is run | `app/core/local_ai_install.py`, `app/core/authenticode.py` | The single most dangerous operation in this product. Ollama's installer is fetched from a host-pinned HTTPS URL; a redirect off the allowlist is refused; the Authenticode signature is verified against Windows' own trust store *and* the signing certificate must name Ollama, because a valid signature alone only proves somebody signed it. The SHA-256 is recorded and shown. A failure at any point deletes the file — there is no "continue anyway". The signature check uses `wintrust.dll`/`crypt32.dll` through ctypes, so there is no command line for a path to be smuggled into. Ollama's installer then runs visibly, under Windows' own elevation prompt, never silently. |
| JARVIS never takes ownership of software it did not install | `app/core/local_ai_install.py`, `app/core/preferences.py` | An Ollama already on the machine is used as it is and never reinstalled over. Whether JARVIS installed it is recorded as `ollama_installed_by_jarvis` so uninstall can tell "we put this here" apart from "this was already here" — removing somebody else's software because JARVIS was uninstalled is the wrong answer every time. |
